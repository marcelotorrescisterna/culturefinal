...
def process_document(doc, doc_id=None):
    ...
    s_doc_id = "\n".join(doc_sent_ids)
    with open(f"individual_parsing/sents/{doc_id}_parsed_lines.txt", "a") as f_out_temp_ind:
        f_out_temp_ind.write(s_doc)
    with open(f"individual_parsing/sent_ids/{doc_id}_parsed_ids.txt", "a") as f_out_temp_ind:
